 ...
 * Compact Logger
 */
export default function CompactLogger (options) {
  // Configure color
  ...
  return new webpack.ProgressPlugin(
    (progress, message, moduleProgress, activeModules, moduleName) => {
      // Reset log output
      logLines = []
      ...
      if (progress >= 0.1 && progress <= 0.7) {
        ...
        if (moduleName !== undefined) {
          ...
          logLines.push(
            chalk.grey(`    ${figures.arrowRight} ${moduleDetails}`)
          )
        }
      ...
      if (progress > 0.7 && progress < 0.95) {
        ...
        const formattedMessageExtra =
          progress === 0.91 ? ' -- may take a while' : '' // Add some extra info (calming devs down)

        logLines.push(
          chalk.grey(
            `    ${figures.arrowRight} ${formattedMessage}${formattedMessageExtra} ...`
          )
        )
